...
# Check if we're on Railway/remote (using DATABASE_URL)
DATABASE_URL = os.environ.get("DATABASE_URL", "")

if IS_RAILWAY or (DATABASE_URL and DATABASE_URL.startswith(('postgres://', 'postgresql://', 'mysql://'))):
    ...
    if DATABASE_URL and DATABASE_URL.startswith(('postgres://', 'postgresql://', 'mysql://')):
        try:
            ...
            print("✅ Using PostgreSQL (remote) via DATABASE_URL")
        except Exception as e:
            print(f"⚠️  Failed to parse DATABASE_URL: {e}")
            print("⚠️  Falling back to SQLite for safety")
        ...
        # Railway but no DATABASE_URL - use PostgreSQL with individual settings
        DATABASES = {
            "default": {
        ...
        print("✅ Using PostgreSQL (remote) with individual settings")
else:
...
# Database Settings - SQLite is used by default in development
# For PostgreSQL in local development (optional), uncomment these:
...
# For Railway/Remote deployment, set DATABASE_URL in Railway dashboard
...
CORS_ALLOW_ALL=True
""")
    print("✅ Created .env file with default values. SQLite will be used for local development!")
...
if not any(arg in sys.argv for arg in ["migrate", "makemigrations", "collectstatic", "flush"]):
    ...
    print(f"Database: {'SQLite (local)' if not (IS_RAILWAY or DATABASE_URL) else 'PostgreSQL (remote)'}")
    if DATABASE_URL and (IS_RAILWAY or DATABASE_URL):
        print(f"Database URL: {'Configured via DATABASE_URL' if DATABASE_URL else 'Using individual settings'}")
